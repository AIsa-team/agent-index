...
> Before first use (or if `${CLAUDE_PLUGIN_ROOT}/.venvs/sec-filings/bin/python` is missing), run:
> `bash "${CLAUDE_PLUGIN_ROOT}/scripts/ensure-venv.sh" sec-filings`
> Then invoke scripts with `${CLAUDE_PLUGIN_ROOT}/.venvs/sec-filings/bin/python`. First run installs dependencies and can take a few minutes.
...
CODE = r'''
import sys
sys.path.insert(0, "${CLAUDE_PLUGIN_ROOT}/skills/sec-filings/scripts")
...
'''
r = subprocess.run(["${CLAUDE_PLUGIN_ROOT}/.venvs/sec-filings/bin/python", "-c", CODE],
...
```bash
${CLAUDE_PLUGIN_ROOT}/.venvs/sec-filings/bin/python ${CLAUDE_PLUGIN_ROOT}/skills/sec-filings/scripts/sec_boot.py            # check config
${CLAUDE_PLUGIN_ROOT}/.venvs/sec-filings/bin/python ${CLAUDE_PLUGIN_ROOT}/skills/sec-filings/scripts/sec_boot.py --set "…"  # save identity
```
...
```bash
${CLAUDE_PLUGIN_ROOT}/.venvs/sec-filings/bin/python ${CLAUDE_PLUGIN_ROOT}/skills/sec-filings/scripts/sec_boot.py
```
...
2. **Save it for them** — one command, correct destination and quoting for this
   ...
   ```bash
   ${CLAUDE_PLUGIN_ROOT}/.venvs/sec-filings/bin/python ${CLAUDE_PLUGIN_ROOT}/skills/sec-filings/scripts/sec_boot.py --set "Their Name their@email.com"
   ```
