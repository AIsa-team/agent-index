...
> Before first use (or if `${PLUGIN_ROOT}/.venvs/dsa/bin/python` is missing), run:
> `bash "${PLUGIN_ROOT}/scripts/ensure-venv.sh" dsa`
> Then invoke scripts with `${PLUGIN_ROOT}/.venvs/dsa/bin/python`. First run installs dependencies and can take a few minutes.
...
result = subprocess.run(
    [
        "${PLUGIN_ROOT}/.venvs/dsa/bin/python",
        "${PLUGIN_ROOT}/skills/finance/daily-stock-analysis/scripts/dsa_port_health.py",
    ],
...
result = subprocess.run(
    [
        "${PLUGIN_ROOT}/.venvs/dsa/bin/python",
        "${PLUGIN_ROOT}/skills/finance/daily-stock-analysis/scripts/dsa_market_brief.py",
        "US",
    ],
...
- Scripts:    `${PLUGIN_ROOT}/skills/finance/daily-stock-analysis/scripts/`
- Shared lib: `_dsa_lib.py` (model routing, yfinance, formatting, report markers)
- Venv python: `${PLUGIN_ROOT}/.venvs/dsa/bin/python` (managed by the platform's `setup.python` step)
...
- Uses the isolated venv behind `${PLUGIN_ROOT}/.venvs/dsa/bin/python`; cannot affect Hermes's own Python environment.
