...
**Script**: `python3 ~/.aisa/agents/cio/portfolio/update_holdings.py`
...
5. **Verify** — tell the user they can run `port` to see the updated live valuation. If execution failed or the user wants to undo: the latest snapshot is in `~/.aisa/agents/cio/portfolio/snapshots/`; rollback = copy the snapshot back to `portfolio_truth.json` (run `cp` only after user confirmation).
